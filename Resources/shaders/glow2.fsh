...
void main(void){
    ...
    float minDimension = min(u_sprite_size.x, u_sprite_size.y);

    vec2 center = vec2(u_sprite_size.x / 2, u_sprite_size.y / 2);

    vec2 bounds = vec2(u_sprite_size.x / minDimension, u_sprite_size.y / minDimension);
    //vec2 uv = gl_FragCoord.xy / minDimension;
    vec2 uv = (2. * gl_FragCoord.xy - center.xy) / u_sprite_size.xy;
    ...
    pointR.x += 0.32 * sin(1.32 * u_time);
    pointR.y += 0.3 * sin(1.03 * u_time);
    pointR.z += 0.4 * sin(1.32 * u_time);
    
    pointG.x += 0.31 * sin(0.92 * u_time);
    pointG.y += 0.29 * sin(0.99 * u_time);
    pointG.z += 0.38 * sin(1.24 * u_time);
    
    pointB.x += 0.33 * sin(1.245 * u_time);
    pointB.y += 0.3 * sin(1.41 * u_time);
    pointB.z += 0.41 * sin(1.11 * u_time);
